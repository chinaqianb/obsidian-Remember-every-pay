...
import FindOrWriteData from "./FindOrWirteData";
import RecordDataIo from "../../filerw/recorddataio";
import {it} from "node:test";

export default class ChooseNeedFindDate extends SuggestModal<string>{
	data:string[]
	plugin:MyPlugin
	path:string
    ...
    async onChooseSuggestion(item: string, evt: MouseEvent | KeyboardEvent): Promise<void> {
		new FindOrWriteData(this.app,this.plugin,item,await new RecordDataIo(this.app, this.plugin).findDateBack(item,this.path),this.path).open()
